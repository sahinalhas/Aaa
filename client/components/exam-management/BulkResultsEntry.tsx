...
interface BulkResultsEntryProps {
  open: boolean;
...
export function BulkResultsEntry({
  open,
  onOpenChange,
  session,
  subjects,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[95vw] max-h-[95vh] overflow-hidden flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Tüm öğrenciler için hızlı sonuç girişi yapın. Tab/Enter ile hızlı geçiş yapabilirsiniz.
          </DialogDescription>
        </DialogHeader>
          ...
          <TabsContent value="table" className="flex-1 flex flex-col overflow-hidden mt-4">
            <div className="flex items-center gap-3 mb-3">
              ...
              <Badge variant="secondary">
                {totalStudentsWithData} / {students.length} Öğrenci
              </Badge>
            </div>
        ...
        <div className="flex justify-between items-center pt-3 border-t">
          ...
          <Button onClick={handleSaveAll} disabled={!hasAnyData || isSaving}>
            {isSaving ? (
              <>
